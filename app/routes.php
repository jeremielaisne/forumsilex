...
$app->get('/profil/{num_profil}', function ($num_profil) use ($app){
    ...
    if (($app['session']->get('rang')==4) || ($app['session']->get('rang')==2 && $id==$num_profil)){
...
$app->get('/xml', function() use ($app){
    ...
    require '../src/modeles/modele_messages.php';
    xml_messages();
    return new Response("Accueil : <a href='/www'>Retour</a></br></br>les liens xml (écriture) : <a href='/www/xml/topic'>Export de tous les topics</a></br><a href='/www/xml/messages'>Export de tous les messages</a></br>les liens xml (relecture): <a href='/www/xml/topic_relecture'>Tous les topics</a></br><a href='/www/xml/messages_relecture'>Tous les messages</a></br>");
});
